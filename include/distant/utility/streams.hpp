...
namespace distant::utility
{
	...
	constexpr bool is_output_stream = std::is_base_of<std::ostream, Stream>::value || std::is_base_of_v<std::wostream, Stream>::value;

	template <typename Stream>
	constexpr bool is_input_stream = std::is_base_of<std::istream, Stream>::value || std::is_base_of<std::wistream, Stream>::value;
