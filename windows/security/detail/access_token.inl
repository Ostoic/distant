...
#include <distant\windows\security\access_token.hpp>
...
namespace distant::windows::security
{
	namespace detail
	{
		HANDLE get_token_impl(windows::detail::process_tag, HANDLE process, DWORD access)
		{
			HANDLE token = NULL;
		...
		HANDLE get_token_impl(windows::detail::thread_tag, HANDLE process, DWORD access, bool self)
		{
			HANDLE token = NULL;
		...
		{
			using dispatch = typename windows::detail::object_dispatcher<KernelObject>::dispatch;

			return access_token(windows::handle(
